...
class ScriptRenderingTest(TestCase):
    def setUp(self):
        ...
        self.template = ScriptTemplate.objects.create(
            name="Test Template",
    ...
    def test_script_rendered_content_missing_variables(self):
        """Test rendering when some variables are missing (Jinja2 defaults to empty string or keeps tag depending on config, usually empty)."""
        execution = RouterScriptExecution.objects.create(
            router=self.router,
            template=self.template,
            variables_used={"ip_address": "10.0.0.1/24"}
        )
        # By default Jinja2 renders missing variables as empty strings
